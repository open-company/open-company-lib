...
(def default-css-color-names
  {:aliceblue "#f0f8ff" :antiquewhite "#faebd7" :aqua "#00ffff" :aquamarine "#7fffd4" :azure "#f0ffff" :beige "#f5f5dc" :bisque "#ffe4c4" :black "#000000"
   ...
   :orangered "#ff4500" :orchid "#da70d6" :palegoldenrod "#eee8aa" :palegreen "#98fb98" :paleturquoise "#afeeee" :palevioletred "#d87093" :papayawhip "#ffefd5"
   ...
   :silver "#c0c0c0" :skyblue "#87ceeb" :slateblue "#6a5acd" :slategray "#708090" :snow "#fffafa" :springgreen "#00ff7f" :steelblue "#4682b4" :tan "#d2b48c" :teal "#008080"
...
(def hex-reg-string "#([a-f0-9]{6}|[a-f0-9]{3})")

(def hex-reg-ex (re-pattern (str "(?i)^" hex-reg-string "$")))

(def color-names-reg-string (string/join "|" (->> default-css-color-names keys (map name))))

(def color-names-reg-ex
  (re-pattern (str "(?i)^(" color-names-reg-string ")$")))

(def color-reg-ex
  (re-pattern (str "(?i)^(" hex-reg-string "|" color-names-reg-string ")$")))

(defn valid-hex-color? [hex-color]
  (let [fixed-hex-color (if (string/starts-with? hex-color "#")
                          hex-color
                          (str "#" hex-color))]
    (re-matches hex-reg-ex fixed-hex-color)))

(defn valid-color-name? [color-name]
  (re-matches color-names-reg-ex color-name))

(defn valid-color? [color-string]
  (or (valid-color-name? color-string)
